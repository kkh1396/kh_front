...
//		System.out.println("@@@@ GET 요청이 들어옴");
		 ...
		 */
		 ...
		 // "키" 데이터를 실수형 변수에 저장하고자 한다면 ?
		 if (!height.isEmpty()) {
			 double h = Double.parseDouble(height);
			 System.out.println("키 : " + h);
		 }
		 
		 // 같은 키값으로 전달된 여러 개의 데이터를 추출
		 // request.getParameterValues("키값"):String[]
		 String[] food  = request.getParameterValues("food"); // 음식(food)
//		 System.out.println(Arrays.toString(food));
		 
		 if (food ==null) {
			 System.out.println("좋아하는 음식이 없습니다.");
		 } else {
			 System.out.println("좋아하는 음식 : "+ String.join(",", food));
		 }
		 ...
		 if (food == null) {
			 ...
			 // 선택된 음식 정보가 있을 경우
			 pw.printf("좋아하는 음식 : <span>%s</span>", String.join("/", food) );
