...
<body>
	...
	<p>
	    ...
	    서버로부터 데이터를 가져와 전체 페이지를 바꾸지 않고
	    일부만 로드하여 변경하도록 하는 기법 <br>
	    
	    기존에는 a태그를 사용하거나, form태그의 submit 요청 방식은
	    "동기식" 요청 => 응답 페이지가 전달되어야 확인 가능      
	</p>
	...
	<h3>* 동기식 vs 비동기식</h3>
	  ...
	  <ul>
	     <li>
	         요청 처리 후 해당 응답 페이지가 전달된 후 다음 작업 가능
	     </li>
	     <li>
	     	 서버에 요청한 결과에 따라 응답이 지연되면 무작정 기다려야 함 
	     	 (지연 시 화면이 하얗게 표시됨 )
	     </li>
	     <li>
	         전체 페이지를 응답하기 때문에 화면에 깜빡거림 현상이 있음 
	     </li>
	  </ul>
	  ...
	  <h4>* 장점</h4>
	   <ol>
	   	 <li>
	   	     현재 페이지를 그대로 유지하면서 추가적인 작업(요청)이 있는 경우 사용
	   	 </li>
	   	 <li>
	   	     요청 시 다른 페이지로 변경되지 않음 => 페이지 깜박거림 없음!
	   	 </li>	   	
	   	 <li>
	   	     요청을 하고 응답(데이터)가 돌아올 떄까지 기다리지 않음
	   	     다른 작업 수행 가능 
	   	 </li>
	   	 <li>
	   	 	 ex) 검색어 자동완성, 아이디 중복체크, 찜하기/해제하기, 추천 
	   	 </li>
	   </ol>
	   ...
	   <ol>
	   	 <li>
	   	 	현재 페이지에 리소스가 지속적으로 쌓임 => 페이지가 느려질 수 있음
	   	 </li>
	   	 <li>
	   	 	오류 발생 시 디버깅 어려움
	   	 </li>
	   	 <li>
	   	    요청 처리 후 응답된 데이터를 가지고 새로운 요소(태그)를 만들어서 
	   	    화면에 표시해야 함 => DOM 요소를 만드는 구문을 잘 익혀둬야 함!
	   	 </li>	   	 
	   </ol>
	   ...
	   <script>
	   		// [전송] 클릭되었을 때 /jqAjax1.do 입력된 값을 전송(요청)
	   		$("#btn1").click(function(){
	    		// 기존 방식 (동기식)
	    	    ...
	    	    $.ajax({
	    	    	...
	    	    	success: function(result) {
	    	    		// 통신 성공 시 실행할 내용
	    	    		// => 응답 데이터(결과)가 result 변수에 담겨져 있을 것임!
	    	    		console.log("** jqAjax1.do 통신 성공 **");
	    	    		console.log(result);
	    	    		
	    	    		// result1 아이디 속성을 가진 요소에
	    	    		//  응답 결과를 표시(출력)
	    	    		$("#result1").html(result) 
	    	    	},
	    	    	 error: function(err){
	   ...
	   <script>
	   		// [전송] 클릭되었을 때 입력된 값(이름, 나이)을 
	   		// 		jqAjax2.do 로 전송(post)
	   ...
	   <button onclick="serchUser();">조회</button>
	   <div id="result3"></div>
	   
	   <script>
	   			// [조회] 클릭되었을 때 입력된 회원 번호를 jqAjax3.do 로 전송 (get)
	   			// const searchUser = () => {}
